...
#ifdef CUSPARSEAPI
// cuSPARSE API errors
static const char* NOT_USED _cudaGetErrorEnum(cusparseStatus_t error)
{
    switch (error)
    {
...
template< typename T >
void check_cublas(T result, char const *const func, const char *const file, int const line)
{
    if (result)
    {
...
template< typename T >
void check_cusparse(T result, char const *const func, const char *const file, int const line)
{
    if (result)
    {
        fprintf(stderr, "cuSPARSE error at %s:%d code=%d(%s) \"%s\" \n",
                file, line, static_cast<unsigned int>(result), _cudaGetErrorEnum(result), func);
        DEVICE_RESET
        // Make sure we call CUDA Device Reset before exiting
        exit(EXIT_FAILURE);
...
#define checkCusparseErrors(val)           check_cusparse ( (val), #val, __FILE__, __LINE__ )
